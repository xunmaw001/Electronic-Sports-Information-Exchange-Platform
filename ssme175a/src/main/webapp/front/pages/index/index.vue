	...
	<view class="uni-padding-wrap">
		...
		<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			...
			<view v-if="queryIndex==0" class="search-form round">
				...
				<input v-model="searchForm.yonghuyonghuming" type="text" placeholder="用户名" ></input>
			</view>
			<view v-if="queryIndex==0" class="action">
				<button @tap="onPageTap('yonghu')" :style="{width:'auto',borderRadius:'40rpx',height:'80rpx',fontSize:'28rpx',color:'#fff',backgroundColor:btnColor[2],borderColor:btnColor[2]}" class="cu-btn shadow-blur round">搜索</button>
			</view>
			<view v-if="queryIndex==1" class="search-form round">
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.yingxiongyingxiongmingcheng" type="text" placeholder="英雄名称" ></input>
			</view>
			<view v-if="queryIndex==1" class="action">
				<button @tap="onPageTap('yingxiong')" :style="{width:'auto',borderRadius:'40rpx',height:'80rpx',fontSize:'28rpx',color:'#fff',backgroundColor:btnColor[2],borderColor:btnColor[2]}" class="cu-btn shadow-blur round">搜索</button>
			</view>
			<view v-if="queryIndex==2" class="search-form round">
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.bisaizhanduimingcheng" type="text" placeholder="战队名称" ></input>
			</view>
			<view v-if="queryIndex==2" class="action">
				<button @tap="onPageTap('bisai')" :style="{width:'auto',borderRadius:'40rpx',height:'80rpx',fontSize:'28rpx',color:'#fff',backgroundColor:btnColor[2],borderColor:btnColor[2]}" class="cu-btn shadow-blur round">搜索</button>
			</view>
			<view v-if="queryIndex==3" class="search-form round">
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.xuanshoumingcheng" type="text" placeholder="名称" ></input>
			</view>
			<view v-if="queryIndex==3" class="action">
				<button @tap="onPageTap('xuanshou')" :style="{width:'auto',borderRadius:'40rpx',height:'80rpx',fontSize:'28rpx',color:'#fff',backgroundColor:btnColor[2],borderColor:btnColor[2]}" class="cu-btn shadow-blur round">搜索</button>
			</view>
		</view>
	...
	export default {
		...
		data() {
			return {
				...
				searchForm:{
					yonghuyonghuming:'',
					yingxiongyingxiongmingcheng:'',
					bisaizhanduimingcheng:'',
					xuanshoumingcheng:'',
				},
				CustomBar: '0',
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.yonghuyonghuming="";
				this.searchForm.yingxiongyingxiongmingcheng="";
				this.searchForm.bisaizhanduimingcheng="";
				this.searchForm.xuanshoumingcheng="";
			},
			//轮播图跳转
			onSwiperTap(e) {
			...
			onPageTap(tableName){
				if(this.queryIndex==0) {
					uni.setStorageSync('indexQueryCondition',this.searchForm.yonghuyonghuming);
					this.searchForm.yonghuyonghuming = '';
				}
				if(this.queryIndex==1) {
					uni.setStorageSync('indexQueryCondition',this.searchForm.yingxiongyingxiongmingcheng);
					this.searchForm.yingxiongyingxiongmingcheng = '';
				}
				if(this.queryIndex==2) {
					uni.setStorageSync('indexQueryCondition',this.searchForm.bisaizhanduimingcheng);
					this.searchForm.bisaizhanduimingcheng = '';
				}
				if(this.queryIndex==3) {
					uni.setStorageSync('indexQueryCondition',this.searchForm.xuanshoumingcheng);
					this.searchForm.xuanshoumingcheng = '';
				}
